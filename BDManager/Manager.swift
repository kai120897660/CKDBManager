...
extension CKDBManager {
    ...
    public static func ck_initTable() {
        let tableName = self.ck_tableName()
        let properties = self.properties
        if let olderProperties = UserDefaults.standard.object(forKey: tableName) as? [String] {
            if olderProperties.count < properties!.count {
                return
            }
            ///创建过，如果有字段更新则更新字段
            for propety in properties! {
                if !olderProperties.contains(propety.name) {
                    DBManager.standard.addTableParameter(tableName, key: propety.name, type: propety.DBType)
                }
            }
        }else {
            ...
            UserDefaults.standard.set(keys, forKey: tableName)

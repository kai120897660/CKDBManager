...
extension CKPropetyDescriptor {
    ...
    static var properties: [PropertyModel]? {
        ...
        let base = self.pointer.advanced(by: is64BitPlatform ? 8 : 11)  // contextDescriptorOffsetLocation
        
        // 相对指针偏移值
        let relativePointerOffset = base.pointee - Int(bitPattern: base)
        
        let descriptor = ContextDescriptor<_ClassContextDescriptor>.init(pointer: UnsafeRawPointer(base).advanced(by: relativePointerOffset).assumingMemoryBound(to: _ClassContextDescriptor.self))
        ...
        return (0..<fields).map { (i) -> PropertyModel in
            ...
            let offset = Int(UnsafePointer<Int>(pointer)[descriptor.fieldOffsetVector + i])//Int(UnsafePointer<Int32>(pointer)[descriptor.fieldOffsetVector * 2 + i])
    ...
    static func getPropertyType(_ type: Any.Type?) -> String {
        ...
        if type == Bool.self || type == Bool?.self {
            ...
            type == Int8.self || type == Int8?.self ||
            type == Int32.self || type == Int32?.self {
            return "INTEGER"
